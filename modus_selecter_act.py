...
from network_user_activity import Network_User
import numpy as np
    ...
    def save(self, acc_test, f1_weighted_test, f1_mean_test, ea_iter=0, type_simple='training', confusion_matrix=0,
        ...
        if type_simple == 'training':
            ...
            if self.config['output']== 'attribute':
                child = ET.SubElement(child_dataset, "acc_attr_mean", acc_attr_mean=str(np.mean(acc_attr_test, axis=0)))
                child = ET.SubElement(child_dataset, "acc_attr_std", acc_attr_std=str(np.std(acc_attr_test, axis=0)))
