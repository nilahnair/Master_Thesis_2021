        ...
        for e in range(self.config['epochs']):
            ...
            for b, harwindow_batched in enumerate(dataLoader_train):
                ...
                if (itera + 1) % self.config['valid_show'] == 0 or (itera + 1) == (self.config['epochs'] * harwindow_batched["data"].shape[0]):
                    ...
                    self.exp.log_scalar("standing_class_neg_val_{}".format(ea_itera), c_neg_val[0], itera)
                    self.exp.log_scalar("walking_class_neg_val_{}".format(ea_itera), c_neg_val[1], itera)
                    self.exp.log_scalar("cart_class_neg_val_{}".format(ea_itera), c_neg_val[2], itera)
                    self.exp.log_scalar("handling_up_class_neg_val_{}".format(ea_itera), c_neg_val[3], itera)
                    self.exp.log_scalar("handling_cen_class_neg_val_{}".format(ea_itera), c_neg_val[4], itera)
                    self.exp.log_scalar("handling_down_class_neg_val_{}".format(ea_itera), c_neg_val[5], itera)
                    self.exp.log_scalar("synch_class_neg_val_{}".format(ea_itera), c_neg_val[6], itera)
                    self.exp.log_scalar("none_class_neg_val_{}".format(ea_itera), c_neg_val[7], itera)
