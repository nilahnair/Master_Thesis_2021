...
class Network_User(object):
            ...
            for b, harwindow_batched in enumerate(dataLoader_train):
                ...
                if self.config['output'] == 'softmax':
                    ...
                    if self.config["fully_convolutional"] == "FCN":
                        ...
                        train_batch_attr=np.zeros([sample.shape[0],self.config['num_attributes']+1])
                        
                        for i in range(0,sample.shape[0]):
                            if sample[i]==self.attrs[sample[i],0]:
                                n=sample[i].item()
                                train_batch_attr[i]= self.attrs[n]
                ...
                if self.config['output'] == 'softmax':
                    ...
                    train_batch_l = train_batch_l.to(self.device, dtype=torch.long) #labels for crossentropy needs long type
                    train_batch_attr=torch.from_numpy(train_batch_attr)
                    train_batch_attr=train_batch_attr.to(self.device, dtype=torch.float) #labels for binerycrossentropy needs float type
                
                # forward + backward + optimize
                
                feature_maps, feature_maps_attr= network_obj(train_batch_v)
                ...
                if self.config['output'] == 'softmax':
                    ...
                    loss_id = criterion_soft(feature_maps, train_batch_l) * (1 / self.config['accumulation_steps'])
                    loss_attr = criterion_sig(feature_maps_attr, train_batch_attr[:, 1:]) * (1 / self.config['accumulation_steps'])
                    loss=loss_id+loss_attr
                    '''
                ...
                if (itera + 1) % self.config['valid_show'] == 0 or (itera + 1) == (self.config['epochs'] * harwindow_batched["data"].shape[0]):
                    ...
                    results_val, loss_val, c_pos_val, c_neg_val = self.validate(network_obj, criterion_soft, criterion_sig)
    ...
    def validate(self, network_obj, criterion_soft, criterion_sig):
        '''
            ...
            for v, harwindow_batched_val in enumerate(dataLoader_val):
                ...
                if self.config['output'] == 'softmax':
                    ...
                    if self.config["fully_convolutional"] == "FCN":
                        ...
                        sample = sample.reshape(-1)
                        test_batch_attr=np.zeros([sample.shape[0],self.config['num_attributes']+1])
                        
                        for i in range(0,sample.shape[0]):
                            if sample[i]==self.attrs[sample[i],0]:
                                n=sample[i].item()
                                test_batch_attr[i]= self.attrs[n]
                ...
                if self.config['output'] == 'softmax':
                    ...
                    test_batch_l = test_batch_l.to(self.device, dtype=torch.long)
                    test_batch_attr=torch.from_numpy(test_batch_attr)
                    test_batch_attr = test_batch_attr.to(self.device, dtype=torch.float)
                    # labels for crossentropy needs long type

                # forward
                predictions, predictions_attr = network_obj(test_batch_v)
                
                if self.config['output'] == 'softmax':
                    loss = criterion_soft(predictions, test_batch_l)
                elif self.config['output'] == 'attribute':
                     loss_id = criterion_soft(predictions, test_batch_l)
                     loss_attr = criterion_sig(predictions_attr, test_batch_attr[:, 1:])
                     loss = loss_id+loss_attr
