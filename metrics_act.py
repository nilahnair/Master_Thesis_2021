...
class Metrics(object):
    ...
    #def __init__(self, config, dev):
        ...
        if self.config['num_attributes'] == 4:
            ...
            self.center= self.atts[0:8,1:]
            self.center= torch.cat((self.center, self.atts[10:11,1:]), 0)
            self.center= torch.cat((self.center, self.atts[15:16,1:]), 0)
        ...
        # Normalize the predictions of the network
           ...
           # predictions=predictions.repeat(9,1,1)
            ...
            for i in range(1, predictions.shape[0]):
                dist = dist_funct(predictions[i], self.center)
